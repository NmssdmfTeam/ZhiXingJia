...
public class ImageUtil {
    ...
    public static File getCompressFile(String path, int max_size) {
        ...
        JLog.i(TAG, "getCompressFile#path:" + path);
        ...
        if (bitmap_org != null && bitmap_org.getByteCount() > 0) {
            JLog.i(TAG, "bitmap_org size:" + bitmap_org.getByteCount() / DIVISOR);
    ...
    public static float calculateMatrixSize(float width, float height) {
        ...
        JLog.i(TAG, "matrix_return:" + matrix_return);
...
//        bitmap.recycle();
        JLog.i(TAG, "bitmap_return size:" + bitmap_return.getByteCount() / DIVISOR + ",width:"
                + bitmap_return.getWidth() + ",height:" + bitmap_return.getHeight());
    ...
    public static Bitmap getCcompressImage(final byte[] data) {
        JLog.i(TAG, "getCompressImage is:" + data);
        if (data == null) {
            return null;
        }
    ...
    public static int calculateInSampleSize(BitmapFactory.Options options, int reqWidth, int reqHeight) {
        ...
        JLog.i(TAG, "in_sample_size:" + in_sample_size);
    ...
    public static File saveTempBitmapFile(@NonNull Bitmap bitmap, @NonNull Bitmap bitmap_old, boolean isScale) {
        ...
        try {
            ...
            bitmap.recycle();
            bitmap_old.recycle();
            JLog.i(TAG, "save bitmap file size:" + f.length() + ",path:" + f.getAbsolutePath());
            return f;
        } catch (Exception e) {
            e.printStackTrace();
            return f;
