    ...
    public void handleError(Throwable e) {
        ...
        if (e instanceof HttpException) { // is http exception
            ...
            switch (error.code()) {
                ...
                case NOT_FOUND:
                    ToastUtil.showMsg( "请求失败,请稍后重试:" + error.code());
                    break;
                case REQUEST_TIMEOUT:
                    ToastUtil.showMsg( "请求超时,请稍后重试:" + error.code());
                    break;
                case INTERNAL_SERVER_ERROR:
                case NOT_IMPLEMENTED: // 服务器不支持访问的方法
                case BAD_GATEWAY:
                    ToastUtil.showMsg( "服务器异常,请稍后重试:" + error.code());
                    break;
                case SERVICE_UNAVAILABLE:
                    ToastUtil.showMsg("服务器正在维护,请稍后重试:" + error.code());
                    break;
                default:
                    ToastUtil.showMsg( "请求失败,请稍后重试:" + error.code());
                    break;
            ...
            ToastUtil.showMsg("网络连接异常，请检查网络");
        } else {
